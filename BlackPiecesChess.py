...
def movement(currently,future):
    if copy_list_chessboard[currently.rank][currently.file] == "bP" and future.rank == 0:
        ...
        global piece_next_location
        piece_next_location += promotion[0].upper()
    else:
...
try:
    ...
    while True:
        while True:
            try:
                ...
                if len(white_piece_currently) == 0:
                    ...
                    time.sleep(15)
                    sys.exit()
                location_currently = Translator(file_to_number.index(white_piece_currently[0]),int(white_piece_currently[1]) - 1)
                location_future = Translator(file_to_number.index(white_piece_future[0]),int(white_piece_future[1]) - 1)
        ...
        while True:
            try:
                print("\n\t\tYOUR MOVE NOW")
                piece_letter = input("\nPiece Type (first letter) ->  ").upper()
                piece_location = input("Piece Location ->  ").lower()
                piece_next_location = input("Where To Move The Piece ->  ").lower()
                black_piece_currently = Translator(file_to_number.index(piece_location[0]),int(piece_location[1]) - 1)
                black_piece_future = Translator(file_to_number.index(piece_next_location[0]),int(piece_next_location[1]) - 1)
                movement(black_piece_currently,black_piece_future)
                print(unfold())
                clientsocket.send(piece_location.encode())
                clientsocket.send(piece_next_location.encode())
                break
            except (IndexError,ValueError):
                print("Wrong Input")
